...
fn main() {
	...
	if clap_matches.is_present("interpret") {
		match clap_matches.value_of("interpret") {
			None => {
				// Enter REPL
			...
			Some(fname) => if let Err(errno) = loader::run_file(fname) {
				println!("{} {}", Red.paint("Run file failed:"), errno);
			}
		}
	}

	if let Some(fname) = clap_matches.value_of("compile") {
		match loader::compile_file(fname) {
